...
def get_system_analytics(db: Session, current_user: User = None) -> Dict[str, Any]:
    ...
    if not current_user or role == "Admin":
        ...
        # Scoped usage: generated by team members
        scoped_usage = db.query(LLMUsage).filter(LLMUsage.user_id.in_(user_ids)).all()
    ...
    # If admin, fetch all usage. Otherwise use scoped_usage
    llm_records = db.query(LLMUsage).all() if (not current_user or role == "Admin") else scoped_usage
