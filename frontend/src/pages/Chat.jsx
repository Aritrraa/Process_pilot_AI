                        ...
                        <div className="agent-steps-panel">
                          {msg.steps.map((step, si) => (
                            <React.Fragment key={si}>
                              {si > 0 && <div className="step-arrow">&#8595;</div>}
                              <div className="agent-step">
                                <span className="step-name">{step.agent}</span>
                                <span className="step-action">{step.action}</span>
                                <span className="step-result" style={{
                                  color: step.result?.includes('Running') || step.result?.includes('Streaming') ? 'var(--color-orange)' : 
                                         step.result?.includes('Failed') ? 'var(--color-red)' : 
                                         step.result?.includes('Completed — no relevant') ? 'var(--text-muted)' : 'var(--color-green)'
                                }}>{step.result}</span>
                              </div>
                            </React.Fragment>
                          ))}
